...
$cmd = "docker exec ${ContainerName} mkdir -p /home/node/.n8n/custom && docker cp /tmp/n8n-nodes-teckel-ipfs ${ContainerName}:/home/node/.n8n/custom/ && docker exec ${ContainerName} sh -c 'cd /home/node/.n8n && rm -rf node_modules/n8n-nodes-teckel-ipfs && npm install ./custom/n8n-nodes-teckel-ipfs --ignore-scripts' && docker restart ${ContainerName} && echo Done"
Invoke-Ssh $cmd
